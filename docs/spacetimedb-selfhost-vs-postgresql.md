...
Admin interface со статистикой активности, платежами, balances, refunds, rake, матчами и спорными случаями естественнее строить поверх PostgreSQL views/queries. В SpacetimeDB это возможно, но сейчас у нас уже видны обходные пути: backend SQL fallback, JSONL audit fallback, ручные SQL checks.
...
На первый взгляд это компромисс, но он опасен, если баланс нужен для stake inside match. Нельзя иметь два источника истины по балансу.
...
- Перед матчем backend резервирует stake в PostgreSQL.
- SpacetimeDB match settlement публикует событие, backend применяет payout в PostgreSQL.
...
5. Прогнать parity tests: full match, max-round settlement, draw rake, timeout, reconnect, payment credit, refund.
